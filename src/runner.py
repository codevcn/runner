    ...
    try:
        ...
        if type_included == None:
            ...
            if action_included == None:
                ...
                open_working_vscode(
                    "cs" if cursor_IDE_included else "code", powershell_only_included
                )
            elif action_included == RUNNER_CODE_TEST:
                open_testing_folder_in_vscode("cs" if cursor_IDE_included else "code")
            elif action_included == RUNNER_CODE_TYPESCRIPT_TEMPLATE:
                open_typescript_template_in_cursor(
                    "cs" if cursor_IDE_included else "code"
                )
            elif action_included == RUNNER_CODE_JS or action_included == RUNNER_CODE_TS:
                open_testing_javascript_typescript_folder_in_vscode(
                    "cs" if cursor_IDE_included else "code"
                )
            elif action_included == RUNNER_CODE_NESTJS:
                open_template_nestjs_folder_in_vscode(
                    "cs" if cursor_IDE_included else "code"
                )
            elif action_included == RUNNER_CODE_PY:
                open_testing_python_folder_in_vscode(
                    "cs" if cursor_IDE_included else "code"
                )
            elif action_included == RUNNER_CODE_EXTENSIONS:
                open_vscode_extensions_in_vscode(
                    "cs" if cursor_IDE_included else "code"
                )
            else:
